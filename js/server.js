...
// example of defining helpers, actual handlebars-helper function for it is 'eq'
...
hbs.registerHelper('if_notUndefNull', function (val, opts) {
...
async function main() {
  ...
  app.get("/search", async (req, res) => {
    const { template_id, material_id } = req.query;
    const [materials] = await connection.execute(`
        SELECT material_id, name
        FROM materials;
    `);
    ...
    if (template_id && material_id) {
      [results] = (await connection.execute(`
        ...
        FROM orders 
          ...
          JOIN templates ON orders.template_id = templates.template_id
          JOIN materials ON orders.material_id = materials.material_id
        WHERE templates.template_id = ${template_id} AND materials.material_id = ${material_id}
      `));
    }
    ...
    let data = {
      ...
      "templates": templates,
      "results": results,
      "selectedTemplate" : template_id === undefined ? -1 : template_id ,
      "selectedMaterial" : material_id === undefined ? -1 : material_id
    }
    console.log(data);
    res.render("search_get", data);
  });
}
